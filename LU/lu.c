 ...
 * A = L * U, onde:
 *  - U é obtida fazendo eliminações em A (escalaonando linhas)
                                       ...
                                       double tolerancia)
{
    ...
    for (int indicePivo = 0; indicePivo < ordemMatriz - 1; indicePivo++) {
        ...
        // Checagem de estabilidade (sem pivotamento)
        if (fabs(valorPivo) < tolerancia) {
            puts(">> Pivô ~ 0: fatoração falhou (sem pivotamento).");
            return LU_PIVO_NULO;
        }
                            ...
                            double* vetorY, int ordemMatriz, double tolerancia)
{
    for (int i = 0; i < ordemMatriz; i++) {
        ...
        double diag = matrizL[i][i];           // em Doolittle diag(L)=1, mas deixo geral
        if (fabs(diag) < tolerancia) return 1; // falha
        vetorY[i] = (vetorB[i] - soma) / diag;
    }
    return 0;
}
                             ...
                             double* vetorX, int ordemMatriz, double tolerancia)
{
    for (int i = ordemMatriz - 1; i >= 0; i--) {
        ...
        double diag = matrizU[i][i];
        if (fabs(diag) < tolerancia) return 1; // pivô ~ 0
        vetorX[i] = (vetorY[i] - soma) / diag;
    }
    return 0;
}
                       ...
                       double** matrizInversa, int ordemMatriz, double tolerancia)
{
    ...
    for (int j = 0; j < ordemMatriz; j++) {
        ...
        if (substituicaoProgressiva(matrizL, vetorB, vetorY, ordemMatriz, tolerancia)) {
            free(vetorB); free(vetorY); free(vetorX); return 2;
        }
        if (substituicaoRegressiva_U(matrizU, vetorY, vetorX, ordemMatriz, tolerancia)) {
            free(vetorB); free(vetorY); free(vetorX); return 3;
        }
                 ...
                 double** matrizInversa)
{
    ...
    LUStatus st = luDecomposicao(matrizA, ordemMatriz, matrizL, matrizU, tolerancia);
    if (st != LU_OK) { 
